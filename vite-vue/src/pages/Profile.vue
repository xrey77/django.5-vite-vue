    ...
    export default defineComponent({
        ...
        methods: {
              ...
              await api.get(`api/getuserid/${this.userid}/`, { headers: {
                ...
                .then((res: any) => {
                        ...
                        if (res.data.qrcodeurl.length > 0) {
                            this.qrcodeurl = res.data.qrcodeurl.slice(2);
                        } else {
                            this.qrcodeurl = 'http://127.0.0.1:8000/static/images/qrcode.png';
                        }
                        setTimeout(() => {
                            this.profileMsg = '';    
                        }, 1000);
            ...
            submitProfile: function() {
                const data =JSON.stringify({id: this.userid, lastname: this.lastname, 
                    firstname: this.firstname, mobile: this.mobile });
    
                api.patch('/api/updateprofile/', data, { headers: {
                Authorization: `Bearer ${this.token}`
                }} )
                .then((res: any) => {
                        this.profileMsg = res.data.message;
            ...
            changePassword: function() {
                ...
                const data =JSON.stringify({id: this.userid, password: this.password });
                api.patch('api/changepassword/', data, { headers: {
                Authorization: `Bearer ${this.token}`
                }} )
                .then((res: any) => {
                        this.profileMsg = res.data.message;
